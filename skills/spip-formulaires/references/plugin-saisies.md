...
// charger() and verifier() are handled automatically by Saisies — only add them
// if you need extra logic beyond what the plugin provides.
...
Multiple verifiers (plugin ≥ 4.3.0):
```php
'verifier' => [
    ['type' => 'email'],
...
Notes :
- `#ENV{_saisies}` avec underscore — évite la conversion en entités HTML du tableau
- Le bouton submit est généré par `inc-saisies-cvt-boutons` via l'option `texte_submit` — ne pas l'écrire manuellement
- `<!--extra-->` est un marqueur que SPIP utilise pour insérer des saisies supplémentaires (extra, bigup…) — à conserver
- Le wrapper AJAX est conditionnel à l'option `ajax` du formulaire
- Pour les formulaires multi-étapes, consulter `formulaires/inc-saisies-cvt-etapes-*.html` dans les sources du plugin

Vous devez aussi appeler `saisies_verifier()` explicitement dans `verifier()` :
```php
