...
([`exec/mutualisation.php:497-521`](https://git.spip.net/spip-contrib-extensions/mutualisation/-/blob/master/exec/mutualisation.php), [`mutualiser_upgradeplugins.php:13-19`](https://git.spip.net/spip-contrib-extensions/mutualisation/-/blob/master/mutualiser_upgradeplugins.php)). On success it
purges broadly: `_DIR_CACHE` recursively, `_DIR_AIDE`, `_DIR_VAR/cache-css`,
`_DIR_VAR/cache-js`, `_DIR_SKELS`, the invalidateurs, `plugin_xml_cache.gz`, and the
`_CACHE_RUBRIQUES` / `_CACHE_CHEMIN` / `_CACHE_PLUGINS_OPT` files
...
The dashboard's **“Upgrader tout”** button (`exec/mutualisation.php:57`, `id='pluginsupgrade'`)
fires this route over AJAX for every site at once. Prefer the checkpointed per-site
sequence: the button has no per-site checkpoint, purges caches before validation, and
gives every site farm-wide blast radius in a single click.
